...
# TODO use `sf data create record --sobject Account --values "Name=Acme" --json | jq --raw-output '.result.id'`
...
alias sf-new-collateral-type='sf data create record --sobject LLC_BI__Collateral_Type__c --values "LLC_BI__Advance_Rate__c=20, LLC_BI__lookupKey__c=my-collateral-type" --json | jq --raw-output ".result.id"'
alias sf-new-collateral-2='key="my-collateral-$RANDOM"; sf data create record --sobject LLC_BI__Collateral__c --values "Name=Acme" --json | jq --raw-output ".result.id"'
